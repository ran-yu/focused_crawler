...
public class Document {
	...
	/**
	 * Constructor for document class that is used by the parent TfIdf class
	 * @param terms loaded as doc name
	 * @param parent the TfIdf class calling this ctor
	 */
	public Document(String terms, TfIdf parent) {
		String line = terms;
		String word;
		StringTokenizer tokens;
		sumof_n_kj = 0;
	...
	public Document(String terms, Set<String> stop_words) {
		String line = terms;
		String word;
		StringTokenizer tokens;
		sumof_n_kj = 0;
		...
		while(tokens.hasMoreTokens()) {
			...
			if (word.length() < 2 || stop_words.contains(word)){
				//System.out.println("Trace stpo words:\t"+word);
				continue;
			}
			if (words.get(word) == null) {
				tempdata = new Double[]{1.0,0.0,0.0};
