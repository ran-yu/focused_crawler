...
public class TfIdf {
	...
	public TfIdf(Map<String, String> seeds,  Map<String, String> candidates){
		allwords = new TreeMap<String, Double[]>();
		documents = new TreeMap<String, Document>();
		docSize = 0;
		...
		{
			insertEntity(entry.getKey().toString(), entry.getValue().toString());
			docSize++;
		}
	...
	public void insertEntity(String uri, String terms) {
		Document doc = new Document(terms, this);
	...
	public double cosSimilarity(Document doc1, Document doc2) {
		...
		if(doc1.vectorlength * doc2.vectorlength == 0){
			similarity = 0;
		}
		else {
	...
	public double jaccardSimilarity(Document doc1, Document doc2) {
		String word;
		double similarity = 0;
		for (Iterator<String> it = doc1.words.keySet().iterator(); it.hasNext(); ) {
			word = it.next();
